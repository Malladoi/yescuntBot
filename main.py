    ...
    try:
        if str(message.text).lower() == 'да':
            bot.send_sticker(message.chat.id,
                             ...
                             message.id)
        if (
                message.reply_to_message.text == 'Напиши мне название города или населенного пункта' and
                message.reply_to_message.from_user.username == 'Winkies_bot'):
            city = message.text
            resp = wthr.weatherbycity(city)
            if resp.status_code == 200:
                dict_str = resp.content.decode("UTF-8")
                ...
                dict_str = resp.content.decode("UTF-8")
                wdata = ast.literal_eval(dict_str)
                bot.reply_to(message, wdata["message"])
    except Exception as e:
        print(e)
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            re = types.InlineQueryResultArticle(id='1',
                                                ...
                                                thumb_url="https://openweathermap.org/img/wn/{0}@2x.png".format(
