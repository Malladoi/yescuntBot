...
def refreshinlineButtonMarkupForecast(message):
    ...
    try:
        if message.text in ["/weather", "/weather@{0}".format(bot_name)]:
            bot.send_message(message.chat.id,
                             ...
                             reply_markup=refreshinlineButtonMarkupForecast(message))
...
def setrespsettings(call):
    ...
    bot.edit_message_reply_markup(chat_id=call.message.chat.id,
                                  message_id=call.message.message_id,
                                  reply_markup=refreshinlineButtonMarkupForecast(call.message))
...
def currWather(call):
    ...
    if resp.status_code == 200:
        ...
        bot.edit_message_text(chat_id=call.message.chat.id,
                              message_id=call.message.id,
                              text=ast.literal_eval(resp.text)['message'])
